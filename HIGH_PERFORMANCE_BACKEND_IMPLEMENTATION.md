...
잠금 파일 기준 wheel 크기는 macOS arm64 PyTorch 80.6MB, TorchVision 1.86MB다. 최종 app·dmg 크기는 M1에서 PyInstaller와 Tauri 빌드를 실행한 뒤 측정해야 한다.
...
1. `system-profile`이 `APPLE_M1_16_BASELINE`, `mps`, PyTorch 2.11.0을 보고하는지 확인한다.
...
| M1 MPS 미검증 | 보수적 batch·FP32·CPU fallback | 실제 M1 장시간 수용 시험 |
...
| unsigned 앱 | 자동 배포 전 단계 | Windows·Apple 코드 서명 |
...
다음 필수 단계는 M1 Pro 16GB에서의 실제 MPS 수용 시험과 실제 촬영 고정 평가 세트 구축이다. 이 두 증거 없이 모델을 `qualified`로 표시하거나 자동 전달에 사용하면 안 된다.
